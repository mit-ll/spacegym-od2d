...
import orbit_defender2d.LBB.default_game_parameters as DGP
import orbit_defender2d.LBB.game_server as GS
from copy import deepcopy
from orbit_defender2d.LBB import lbb
from orbit_defender2d.LBB.examples.server_utils import \
...
# Game Parameters
GAME_PARAMS = lbb.LBBGameInputArgs(
...
class PlayerClient(object):
    ...
    def __init__(self, router_addr, pub_addr, plr_alias, sub_topic=''):
        ...
        Args:
            ...
            plr_alias : str
                alias used for registered player in LBB game
            sub_topic : str
    ...
    def send_random_action_req(self, context):
        ...
        if context == U.DRIFT:
            ...
            for tok in self.game_state[GS.TOKEN_STATES]:
                if lbb.parse_token_id(tok[GS.PIECE_ID])[0] == self.player_id:
...
def run_server_2p_1ran_1remote_game():
    # create game object
    game = lbb.LBBGame(**GAME_PARAMS._asdict())
